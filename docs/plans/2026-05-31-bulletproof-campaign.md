...
| P2b | ✅ committed | d3b4004 (+ test fix ae0c3c5) | atomic `CompleteWithResult`/`FailTerminalWithResult` on *GormStorage (type-asserted, no core.Storage change): weld the fan-out counter increment into the ownership-guarded state-transition tx, liveness-guarded `WHERE fan_outs.status='pending'`; move `cancelHeartbeat()` AFTER the completion write; prompt `Release` on transient fail. Closes SaveJobResult-ok/Complete-fail wedge, reaper-vs-Complete dup-run, and transient-fail mis-count. Gate (all green): build/vet + 8 storage RED→GREEN (incl LivenessGuard) + 4 worker fault-injection + -race storage/worker + multi-backend PG(41.9s)/MySQL(21.3s) `-p=1` + ./tests/ integration (294.6s) + chaos all-HARD-PASS (INV-FANOUT-COUNTS 20/0) + 10x APPROVE-WITH-NITS. Design+proof: docs/plans/2026-05-31-p2b-design.md. Test-portability fix ae0c3c5: `p2bID` helper built ids >varchar(36) → passed sqlite, failed PG/MySQL (SQLSTATE 22001); shortened to suffix+UnixNano. Nits→P2c (non-blocking): dedup the two methods; add ConcurrentWithReaper test; comment Release call-site (at-least-once). |
| P3 | ✅ committed | 3755d9f | scheduler catch-up: GetScheduledFireTime via optional scheduledFireReader interface (no core.Storage churn); seed lastRun from persisted LastFireAt after fleet gap, anti-boot-storm preserved. Gate: race + 2 RED→GREEN + PG/MySQL + chaos INV-SCHED green. **10x APPROVE-WITH-NITS (no must-fix).** Nit→P3b. |
| P3b | deferred | — | 10x-review nit: catch-up fires one boundary per 100ms tick, so a SHORT-interval Every() down a LONG time replays many boundaries at ~10/sec (bounded + fleet-deduped by ClaimScheduledFire — NO duplicate exec, NO wedge; large-interval Daily/Weekly/Cron fire ≤1-2). Fix: clamp the seed to the most-recent missed boundary (true fire-once-then-resume). Low-med sev; touches worker.go (serial). Also a nowFunc clock seam would de-flake the scheduler test family (pre-existing).
...
| P8 | ✅ committed | 3f40afa + 3bccdd1 + 9f8e8c8 | UI: PurgeQueue rejects empty name/status (+isValidPurgeStatus), PurgeJobs cascades checkpoints in tx, ListWorkflows excludes cancelled from running, SearchJobs/GetWorkflowRoots clamp offset/limit. Gate: full suite 14 ok/0 fail incl ui race + tests/ integration + PG/MySQL. **10x REVIEW must-fix (9f8e8c8): the re-applied codex allowlist wrongly permitted running/retrying/waiting (would orphan worker locks / corrupt fan-out) — opposite of its doc; original test only checked empty/bogus so it slipped. Restricted to terminal/idle states + added reject-test. NOW matches contract.** **INCIDENT (resolved): P5/P8 ran as concurrent codex dispatches; P5's iter-3 cleanup reverted P8's ui/service.go to HEAD in the shared tree. First P8 commit 3f40afa shipped gorm_ui+tests but NOT service.go impl → HEAD red; two intervening docs commits compounded it; recovered by re-applying the original codex P8 service.go diff in 3bccdd1 (verified ui green + tests/ integration green). Lessons (in memory): (1) NEVER run concurrent codex in shared tree — serialize or isolation:worktree; (2) re-run named acceptance tests immediately before EVERY commit, abort if red; (3) Edit anchors must come from a fresh Read of current HEAD, not assumptions.** Read-path redaction + dead q.events deferred to P8b. |
...
| P7a | ✅ committed | 3c79c8e | durable error rehydration fidelity: CheckpointErrorKind identity-checks (errors.Is) before tagging sentinel; RehydrateCheckpointError fully kind-driven, no message-guessed sentinel swap, replay-idempotent. Gate: race pkg/core+pkg/call + 4 RED→GREEN + updated enshrining coverage tests. (Closes rehydrate-sentinel-substitution, rehydrate-double-wrap-loss.) |
| P7b | ✅ committed | a1c9f3e | durable result-size cap (security.MaxResultSize 1MB + NoRetry at handler.Execute & Call before checkpoint persist) + acceptsEmptyArgs for slice/map/ptr/interface/empty-struct. Gate: race handler+call+security + 4 RED→GREEN. (Closes checkpoint-result-size-unbounded, empty-args-noretry-valid-empty-types.) |
| P7c | deferred | — | doc-only: concurrent-Call determinism contract (single-goroutine), phase-name uniqueness, besteffort stale-checkpoint, Call/at-least-once; + error-only-handler-via-Call[T] guard (needs design — erroring may break Call[any] on error-only handlers). Low risk; fold into a docs pass or P9. |
| P8 (dup row) | — | — | superseded by P8 row above |
| P9 | ✅ committed | (this commit) | matchable core.ErrJobNotFound sentinel wrapped in LoadResult/LoadStatus + re-exported; Timezone field documented as reserved/ignored; pause.ResumeJob documented as storage-only (no event). Gate: race core+queue+root + 1 RED→GREEN (TestErrJobNotFound_IsMatchable). (Closes loadresult-loadstatus-not-found-not-sentinel; documents timezone-option-dead + resume-job-facade-divergence.) Deferred to P9b: Sub() explicit-zero-priority (fanout.go treats Priority(0) as unset — needs care vs default fallback). |
