...
| P7b | dispatching | — | durable Call result-size cap (MaxResultSize + NoRetry on oversize) + empty-args accepted for nilable arg kinds. (Deferred from P7: error-only-handler-via-Call[T] guard — erroring breaks documented Call[any] on error-only handlers, needs design; concurrent-Call determinism + phase-name-uniqueness + besteffort-stale-checkpoint → doc-only, fold into P9/P7c.) |
