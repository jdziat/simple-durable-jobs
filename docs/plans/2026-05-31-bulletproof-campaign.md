...
| P3 | ✅ committed | 30e6f88 | scheduler catch-up: GetScheduledFireTime via optional scheduledFireReader interface (no core.Storage churn); initialScheduleSeed replays one missed boundary after fleet gap, anti-boot-storm preserved. Gate: race + 2 RED→GREEN (CatchUpAfterFleetGap +_NoCatchUpForFreshSchedule, GetScheduledFireTime +_NotFound) + PG/MySQL + chaos INV-SCHED green. |
