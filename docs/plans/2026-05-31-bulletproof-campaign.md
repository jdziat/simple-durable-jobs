...
| P8 | ✅ committed | 3f40afa + 9a8f0f4 | UI: PurgeQueue rejects empty name/status (+isValidPurgeStatus), PurgeJobs cascades checkpoints in tx, ListWorkflows excludes cancelled from running, SearchJobs/GetWorkflowRoots clamp offset/limit. Gate: ui+storage race + 4 RED→GREEN + PG/MySQL. **INCIDENT: P5/P8 ran as concurrent codex dispatches; P5's cleanup reverted P8's ui/service.go in the shared tree → first P8 commit (3f40afa) was missing service.go impl + HEAD went red; recovered by hand-reimplementing F1/F3 in 9a8f0f4. Lesson: serialize codex dispatches or use isolation:worktree; re-run named tests before every commit.** Read-path redaction + dead q.events deferred to P8b. |
