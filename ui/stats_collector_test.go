...
func TestStatsCollector_EventDrivenCounters(t *testing.T) {
	...
	// Emit events
	job := &core.Job{ID: uiTestUUID("test-1"), Queue: "default", Type: "test"}
...
func TestStatsCollector_MultipleQueues(t *testing.T) {
	...
	q.Emit(&core.JobCompleted{Job: &core.Job{ID: uiTestUUID("1"), Queue: "emails", Type: "send"}, Timestamp: time.Now()})
	q.Emit(&core.JobCompleted{Job: &core.Job{ID: uiTestUUID("2"), Queue: "payments", Type: "charge"}, Timestamp: time.Now()})
	q.Emit(&core.JobFailed{Job: &core.Job{ID: uiTestUUID("3"), Queue: "emails", Type: "send"}, Timestamp: time.Now()})
...
func TestStatsCollector_Snapshot(t *testing.T) {
	...
	_ = store.Enqueue(ctx, &core.Job{
		ID: uiTestUUID("snap-1"), Queue: "snap-queue", Type: "work", Status: core.StatusPending,
	})
	_ = store.Enqueue(ctx, &core.Job{
		ID: uiTestUUID("snap-2"), Queue: "snap-queue", Type: "work", Status: core.StatusPending,
	})
