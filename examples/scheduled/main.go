...
func main() {
	// Setup database. SafeSQLiteDSN applies the recommended file-based SQLite
	// settings for concurrent workers.
	db, err := gorm.Open(sqlite.Open(jobs.SafeSQLiteDSN("scheduled.db")), &gorm.Config{})
	if err != nil {
		log.Fatal(err)
	}
